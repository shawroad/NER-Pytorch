    ...
    def get_ent2token_spans(self, text, entity_list):
        ...
        for ent_span in entity_list:
            ...
            token_end_index = list(
                filter(lambda x: token2char_span_mapping[x][-1] - 1 == ent_span[1], token_end_indexs))
...
class MetricsCalculator(object):
    ...
    def get_evaluate_fpr(self, y_pred, y_true):
        # y_pred: batch_size, 10, max_len, max_len
        # y_true: batch_size, 10, max_len, max_len
        y_pred = y_pred.cpu().numpy()
        y_true = y_true.cpu().numpy()
        pred = []
        true = []
        for b, l, start, end in zip(*np.where(y_pred > 0)):
            pred.append((b, l, start, end))   # 遍历每一个样本 拿出10个实体的起始和终止
        for b, l, start, end in zip(*np.where(y_true > 0)):
            true.append((b, l, start, end))   #
        # print(pred[0])   # (0, 0, 0, 1)
        # print(true[0])   # (0, 0, 16, 17)
        R = set(pred)
        T = set(true)
        X = len(R & T)
        Y = len(R)
